...
/**
 * 第二页，自定义listview，无需被BottomScrollView包裹
 */
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (scrollMode == MODE_IDLE) {
                ...
                Log.d("leon","CustListView onInterceptTouchEvent downY:"+downY+" getRawY:"+ev.getRawY());
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (scrollMode == MODE_IDLE) {
                float xDistance = Math.abs(downX - ev.getRawX());
                float yDistance = Math.abs(downY - ev.getRawY());
                Log.d("leon","CustListView dispatchTouchEvent downY:"+downY+" getRawY:"+ev.getRawY());
                if (xDistance > yDistance && xDistance > mTouchSlop) {
                    scrollMode = MODE_HORIZONTAL;
                    //横向滑动交给父控件处理
                    getParent().requestDisallowInterceptTouchEvent(needHorizontalScroll);
